...
func _process(_delta):
	...
	if initHeight < targetHeight:
		if activated and height < targetHeight * 64:#Input.is_action_pressed("ui_up"):
			...
			height += SPEED
			updateHeight()
		elif !activated and height > initHeight * 64:
			$MovingPart.translate(Vector2(0,SPEED)) 
			for body in bodies:
				if body is KinematicBody2D: 
					body.move_and_collide(Vector2(0,SPEED))
		...
		if !activated and height < initHeight * 64:
			$MovingPart.translate(Vector2(0,-SPEED))
			for body in bodies:
				if body is KinematicBody2D: 
					body.move_and_collide(Vector2(0,-SPEED))
			...
			height += SPEED
			updateHeight()
		elif activated and height > targetHeight * 64:
			$MovingPart.translate(Vector2(0,SPEED)) 
			for body in bodies:
				if body is KinematicBody2D: 
					body.move_and_collide(Vector2(0,SPEED))
